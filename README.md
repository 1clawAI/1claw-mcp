...
| `lease_bankr_key`      | Provision a short-lived Bankr wallet API key (`bk_usr_`) from the org's partner key. Returns lease ID, key, wallet, and expiry. Requires `BANKR_PARTNER_KEY` on Vault. |
