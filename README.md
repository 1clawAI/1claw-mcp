...
| `ONECLAW_BASE_URL`        | No             | `https://api.1claw.co` | Vault API base URL. Agents with `intents_require_tee` / `execution_require_tee` need no special value here: the vault forwards their intents to Shroud itself (vault ≥ 0.61.19). Set it to Shroud (`https://shroud.1claw.co`) only for a DPoP-bound token or a self-hosted deployment that routes signing there. |
...
The server ships **160 tools** (add `proxy_request` in local daemon mode) but a session only ever sees the **toolsets it is entitled to** — a vault-only agent is offered ~24 tools, not 160. **`inspect_content`** also runs in **`ONECLAW_LOCAL_ONLY=true`** mode without vault credentials. Also includes the `vault://secrets` resource.
