...
export function simulateTransactionTool(client: OneClawClient) {
  return {
    ...
    execute: async (
      ...
      if (!agentId) {
        throw new UserError(
          "simulate_transaction requires agent authentication (ONECLAW_AGENT_API_KEY, or ONECLAW_AGENT_ID + key; agent ID is resolved from token exchange when omitted).",
        );
      }
