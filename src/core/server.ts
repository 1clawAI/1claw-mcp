/**
 * Server bootstrap shared by the umbrella package (`index.ts`, every
 * toolset) and the split packages (`packages/vault`, `packages/guard`).
 * Which toolset modules and which client class a server gets are the
 * entrypoint's decision; nothing here imports a domain.
 */
...
export interface ServerConfig {
    /** MCP server name reported on initialize. */
    name: string;
    version: string;
    /** Toolset modules this server offers, in registration order. */
    modules: readonly ToolsetModule[];
    /** Constructs the vault client for a credential (full facade or a narrower one). */
    createClient: ClientFactory;
    /** Force inspect-only mode regardless of env (the guard package). */
    localOnly?: boolean;
    /** Override the initialize `instructions` text. */
    instructions?: string;
}

/** Build, configure and start the server. Resolves once it is listening. */
export async function startServer(cfg: ServerConfig): Promise<{ server: FastMCP<SessionAuth> }> {
installToolsets(cfg.modules);
installClientFactory(cfg.createClient);
...
function registerTool(factory: AnyToolFactory) {
    const proto = factory(
        cfg.createClient({ baseUrl, token: "", vaultId: "" }),
    );
...
if (transport === "httpStream") {
    ...
    console.log(`${cfg.name} MCP server listening on port ${port} (HTTP streaming)${localOnly ? " [local-only mode]" : ""}`);
} else {
    await server.start({ transportType: "stdio" });
    if (localOnly) {
        console.error(`${cfg.name} MCP server started in local-only mode (security tools only, no vault credentials required)`);
    }
}
